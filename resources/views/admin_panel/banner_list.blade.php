                                ...
                                <tbody>
                                    @if(count($record_list) > 0)
                                        @foreach($record_list as $record)
                                            <tr>
                                                <td>{{$record->line_first}}</td>
                                                <td>{{$record->line_second}}</td>
                                                <td><img src="{{asset('storage/banner_images')}}/{{$record->banner_image}}" width="100" /></td>
                                                <td>{{date('d F, Y H:i A', strtotime($record->created_at))}}</td>
                                                <td class="text-center">
                                                    <ul class="table-controls">
                                                        <li><a href="{{route('banner.edit',base64_encode($record->banner_id))}}"  data-toggle="tooltip" data-placement="top" title="Edit"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-check-circle text-primary"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg></a></li>
                                                        <li>
                                                            <form action="{{route('banner.destroy',base64_encode($record->banner_id))}}" method="POST">
                                                                @csrf
                                                                @method('DELETE')
